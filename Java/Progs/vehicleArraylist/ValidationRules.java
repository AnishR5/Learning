...
public class ValidationRules {
	...
	public static Vehicle validateAllValidations(String chasisno, String company, String color, double baseprice,
			String mfgDate, ArrayList<Vehicle> vehicles) throws VehicleInvalidException {
		duplicateChasisno(chasisno, vehicles);
		Color validcolor = parseandvalidatecolor(color);
		double netsal = baseprice + validcolor.getPrice();
		LocalDate validdate = parseandvalidatemgfdate(mfgDate);

		return new Vehicle(chasisno, company, validcolor, netsal, validdate);
	}

	public static void duplicateChasisno(String newChasisno, ArrayList<Vehicle> showroom)
			throws VehicleInvalidException {
